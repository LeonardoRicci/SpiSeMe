...
%	inter-event intervals, Chaos 29 (2019), 121102, <a href="matlab:web('https://doi.org/10.1063/1.5138250')">doi:10.1063/1.5138250</a>.
%
%	Citing this source is highly appreciated. Thank you.
	...
	[h2counts, h2Xedges, h2Yedges] = histcounts2(sequenceOriginalRanks(1:N-1), sequenceOriginalRanks(2:N), [nBins nBins]);
	if (nBins < 2)
	...
	for iter = 1:M
		...
		sequenceSurrogateRanks = zeros(N, 1);
		rng('shuffle', 'twister');
		...
		while (n < N)
			...
			lastExtractedBin = discretize(sequenceSurrogateRanks(n), h2Xedges);
			conditionalDistribution = h2counts(lastExtractedBin, :);
